...
static void *m_rmm_pci_doe_context = NULL;
...
static bool rmm_configure_spdm_version(void *spdm_context, bool is_requester)
{
...
static bool rmm_provision_responder_certificate(void *spdm_context)
{
    libspdm_data_parameter_t parameter;
    void *data;
    size_t data_size;
    ...
    if (!res) {
        printf("RMM_DEMO: Failed to read responder certificate chain (hash=0x%x, asym=0x%x)\n",
               base_hash_algo, base_asym_algo);
        return false;
    }
    printf("RMM_DEMO: Successfully read responder certificate chain, size=%zu\n", data_size);
...
static bool rmm_provision_requester_peer_root_cert(void *spdm_context)
{
    libspdm_data_parameter_t parameter;
    void *data;
    size_t data_size;
    ...
    if (!res) {
        printf("RMM_DEMO: Failed to read responder root certificate (hash=0x%x, asym=0x%x)\n",
               base_hash_algo, base_asym_algo);
        return false;
    }
    printf("RMM_DEMO: Successfully read responder root certificate, size=%zu\n", data_size);
...
static libspdm_return_t rmm_get_response_vendor_defined_request(
    ...
    size_t request_size, const void *request, size_t *response_size, void *response)
{
    return pci_doe_get_response_spdm_vendor_defined_request(
        m_rmm_pci_doe_context, spdm_context, session_id,
        request, request_size, response, response_size);
}

static bool rmm_init_tdisp_responder(void *spdm_context)
{
    libspdm_return_t status;

    libspdm_register_get_response_func(spdm_context,
        rmm_get_response_vendor_defined_request);

    status = pci_doe_register_vendor_response_func(
        m_rmm_pci_doe_context,
                           ...
                           bool is_requester)
{
    ...
    if (is_requester) {
        libspdm_register_device_io_func(context->spdm_context,
                                        rmm_requester_send_message,
                                        rmm_requester_receive_message);
    } else {
        libspdm_register_device_io_func(context->spdm_context,
                                        rmm_responder_send_message,
                                        rmm_responder_receive_message);
    }

    libspdm_register_transport_layer_func(context->spdm_context,
                                          RMM_MAX_SPDM_MSG_SIZE,
    ...
    libspdm_register_device_buffer_func(context->spdm_context,
                                          RMM_SENDER_RECEIVER_BUFFER_SIZE,
                                          RMM_SENDER_RECEIVER_BUFFER_SIZE,
                                          rmm_device_acquire_sender_buffer,
                                          rmm_device_release_sender_buffer,
                                          rmm_device_acquire_receiver_buffer,
                                          rmm_device_release_receiver_buffer);
    ...
    if (!rmm_configure_spdm_version(context->spdm_context, is_requester)) {
    ...
    if (!is_requester) {
        if (!rmm_provision_responder_certificate(context->spdm_context)) {
            free(context->scratch_buffer);
            free(context->spdm_context);
            context->spdm_context = NULL;
            context->scratch_buffer = NULL;
        ...
        if (!rmm_init_tdisp_responder(context->spdm_context)) {
            free(context->scratch_buffer);
            free(context->spdm_context);
            context->spdm_context = NULL;
            context->scratch_buffer = NULL;
        ...
        if (!rmm_provision_requester_peer_root_cert(context->spdm_context)) {
            free(context->scratch_buffer);
            free(context->spdm_context);
            context->spdm_context = NULL;
            context->scratch_buffer = NULL;
